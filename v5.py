...
def cut_num(img):
    ...
    blobs=img.find_blobs([black_threshold],roi=(0,0,320,180),x_stride=25,y_stride=30,invert=1,area_threshold=20)
    ...
    return roi
def cut_num_init(img):
    #img = sensor.snapshot().binary([black_threshold])
    roi=[]

    blobs=img.find_blobs([black_threshold],roi=(50,0,220,200),x_stride=30,y_stride=30,invert=1,area_threshold=30)
    #blobs=img.find_blobs([black_threshold],roi=(0,0,320,120),x_stride=15,y_stride=15,invert=1,area_threshold=10)
...
def rec_num():
    result=-1
    roi=[]
    img = sensor.snapshot().binary([black_threshold]).lens_corr(strength = 1.3, zoom = 0.9)
    ...
    if rect_num==1 or rect_num==2 or rect_num==4:
        ...
        for i in range (0,rect_num):
            ...
            num0446=density(img,x,y,w,h,0/8,4/8,4/8,6/8)
            num0202=density(img,x,y,w,h,0/8,2/8,0/8,2/8)
            num4746=density(img,x,y,w,h,9/16,13/16,9/16,13/16)
            num3556=density(img,x,y,w,h,6/16,10/16,9/16,11/16)
            num6823=density(img,x,y,w,h,12/16,16/16,5/16,7/16)
            num2478=density(img,x,y,w,h,4/16,8/16,13/16,16/16)
            #print("num1",num4823)
            #print("num2",num0335)
            #print("num3",num0246)
            #print("num4",num0246)
            ...
            if num3556<0.2 and num0335<0.1:
                result.append([x,3])
            elif num3556>0.3 and num4746>0.3 and num0202<0.15 and num2478<0.15:
                result.append([x,4])
            elif num2556>0.2 and num0335<0.2 and num7868<0.15:
                result.append([x,7])
            elif num3556<0.2 and num0446<0.3 and num4823<0.2:
                result.append([x,5])
            elif num0135>0.8 and num6823<0.3:
                result.append([x,6])
            elif num0135<0.7 and num6823>0.3:
                result.append([x,8])
            else:
                result.append([x,0])
            #print(result)
...
def rec_num_init():
    ...
    for i in range(0,20):
        for j in range(0,240):
            ...
            img.set_pixel(i+300,j,255)
    #img.lengths_corr(DISTORTION_FACTOR)  #进行镜头畸形矫正，里面的参数是进行鱼眼矫正的程度
    img.dilate(1)
    img.erode(1)
    img.dilate(1)
    roi=cut_num_init(img)
    #print(micropython.mem_info())
    rect_num=int(len(roi)/4)
    #print("ge",rect_num)
    if rect_num==1 or rect_num==2 or rect_num==4:
        ...
        for i in range (0,rect_num):
            ...
            num3556=density(img,x,y,w,h,3/8,5/8,5/8,6/8)
            num0335=density(img,x,y,w,h,0/8,3/8,3/8,5/8)
            num0325=density(img,x,y,w,h,0/8,3/8,2/8,5/8)
            num7868=density(img,x,y,w,h,7/8,8/8,7/8,8/8)
            ...
            num0867=density(img,x,y,w,h,12/16,14/16,0/8,8/8)
            num0246=density(img,x,y,w,h,0/8,2/8,4/8,6/8)
            num3523=density(img,x,y,w,h,3/8,5/8,2/8,3/8)
            #print("num1",num4823)
            #print("num2",num0335)
            #print("num3",num0867)
            #print("num4",num0246)
            #print("num5",num3523)
            if h>2.3*w or (num0867>0.65 and num0246<0.15 and num3523>0.2):
                result.append([x,1])
            elif num3556<0.1 and num0335<0.1:
                result.append([x,3])
            elif num3556>0.4 and num0335>0.3:
                result.append([x,4])
            elif num2556>0.1 and num0325<0.2 and num7868<0.1:
                result.append([x,7])
